...
| 鉴权 | 除 `GET /api/health` 与 `/api/auth/*` 外,所有接口需请求头 `Authorization: Bearer <token>`(登录后获取);缺失/无效/过期返回 `401 {"detail": "..."}`;登录流程见 3.7 |
| 数据隔离 | 所有业务数据按用户隔离:单词本、对话、自测、阅读文章、自定义作文题均为每用户独立;内置作文题库(user_id 为空)全员共享 |
...
请求同 register。响应同 register(`token` + `user`)。
- 邮箱或密码错误:401;连续失败 5 次锁定账号 5 分钟:403
- 锁定期间即使密码正确也返回 403

**`POST /api/auth/logout`**(需鉴权)— 吊销当前 token → `{"ok": true}`

**`GET /api/auth/me`**(需鉴权)— 当前用户信息,响应为 `user` 对象(同 register)

**`POST /api/auth/reset/request`** — 请求重置密码邮件

请求:`{"email": "you@example.com"}` → 响应:`{"message": "如果该邮箱已注册,重置邮件将发送至您的邮箱,请查收"}`

- **防枚举**:邮箱是否存在均返回同一文案
- 每邮箱 60s 限 1 封:429;邮件服务未配置(RESEND_API_KEY 缺失)或发送失败:503
- 邮件内含一次性链接 `${FRONTEND_URL}/reset-password?token=...`(15 分钟有效)

**`POST /api/auth/reset/confirm`** — 设置新密码

请求:`{"token": "<邮件链接中的 token>", "new_password": "password456"}` → `{"ok": true}`

- token 无效/过期:400;成功后该用户**全部会话被吊销**,需重新登录
- token 一次性,用后即失效
